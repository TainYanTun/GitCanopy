...
const highlight = (code: string, fileName: string): string => {
  ...
  const patterns = [
    { name: 'comment', re: /(\/\/.*|\/\*[\s\S]*?\*\/)/ },
    { name: 'string', re: /(['"`])(?:\\.|(?!\1)[^\\\n])*\1/ },
    { name: 'keyword', re: /\b(const|let|var|function|return|if|else|for|while|switch|case|break|continue|import|export|from|class|extends|async|await|try|catch|finally|new|this|typeof|instanceof|interface|type|enum|namespace|as|keyof|readonly|public|private|protected|static|abstract|implements|true|false|null|undefined|void|any|number|string|boolean|object|Symbol|BigInt)\b/ },
...
const parseSplit = (diffContent: string): SideBySideRow[] => {
  ...
  const flushBuffers = () => {
    ...
    for (let i = 0; i < max; i++) {
      rows.push({
        ...
        right: rightBuffer[i] || { lineNumber: null, type: "empty", content: "" },
        key: rows.length
      });
    }
    leftBuffer = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (firstChar === "+") {
      newLineCounter++;
      rightBuffer.push({ lineNumber: newLineCounter, type: "addition", content: line.substring(1) });
    } else if (firstChar === "-") {
      oldLineCounter++;
      leftBuffer.push({ lineNumber: oldLineCounter, type: "deletion", content: line.substring(1) });
    } else {
      flushBuffers();
...
self.onmessage = (e) => {
  ...
  try {
    const result = splitView ? parseSplit(diffContent) : parseUnified(diffContent);
    
    // Apply highlighting to result
    result.forEach(row => {
      if (row.type === 'split') {
        if (row.left?.content && row.left.type !== 'hunk' && row.left.type !== 'info') {
          row.left.content = highlight(row.left.content, fileName);
        }
        if (row.right?.content && row.right.type !== 'hunk' && row.right.type !== 'info') {
          row.right.content = highlight(row.right.content, fileName);
        }
      } else if (row.unified) {
        if (row.unified.content && row.unified.type !== 'hunk' && row.unified.type !== 'info') {
